...
var dataMapload;
var nowDate;
var json = require('../Json/API_KEY.json');
var API_KEY = json.api;
...
const user_data = {
  mochi: {
    data: "2020/03/02", unsei: "大吉", camera: "Eos 5D"
  }
}
        ...
        $.ajax({
          ...
          for (let tennki = 0;
            ...
            switch (data.list[tennki].weather[0].main) {
              case "Clear": tenkiaa += ":sunny:";
                break;
              case "Clouds": tenkiaa += ":cloud:";
                break;
              case "Rain": tenkiaa += ":rain_cloud:";
                break;
              case "Snow": tenkiaa += ":snowman:";
                break;
              default: tenkiaa += data.list[tennki].weather[0].main + "\n\n_*!!実装されてないものがありますこのBOTを作った製作者に伝えてください!!*_\n\n";
                break;
            }
